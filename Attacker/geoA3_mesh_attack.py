...
def attack(net, input_data, cfg, i, loader_len, saved_dir):
    ...
    trg_mesh = Meshes(verts=vertice, faces=faces_idx).cuda()
    src_mesh = trg_mesh.clone().cuda()
        ...
        for step in range(cfg.iter_max_steps):
            ...
            with torch.no_grad():
                ...
                for k in range(b):
                    ...
                    # shape metric
                    batch_k_src_meshes = join_meshes_as_batch([src_mesh[k]]*eval_num)
